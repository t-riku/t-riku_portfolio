...
PageSpeed Insightsを用いて高得点を出すように改善を繰り返していますが、トップページに動画を用いているのもあり、携帯サイズでの初期表示が遅くなってしまっていて、ユーザー補助、おすすめの方法、SEOは高得点を出しているのですが、パフォーマンスが悪いので今後改善していきます。

この制作からバニラのJSではなかなかできない、Reactならではの仮想DOM、コンポーネントの考え方、高速稼働、様々なパッケージの利用などに惹かれてモダンな技術に興味を持ち学習することになりました。
