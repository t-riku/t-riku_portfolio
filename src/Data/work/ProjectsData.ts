...
const projectsData = [
  ...
  },
  {
    ...
    category: "Web",
    time: "2022.6",
    text: `主にReact×Firebaseで両親の建築事務所のHPを一から制作しました。
...
「有用性」としてはこのポートフォリオサイトを見に来てくれたユーザーが満足できる内容にするために「トップページ(Top)」「住宅事例(Works)」「ご挨拶(Message)」「レビューリンク(Review)」「コンタクト(Contact)」「理念(Philosophy)」「設計の流れ(Flow)」「事務所概要(Overview)」セクションを配置し、有用性の高い見たいであろう情報がしっかり入っているサイトにしました。またお客さんが一番見る作品集の情報を充実させたいと考え、下のMoreボタンを押すとその家の詳細が見れるモーダルが開くようになっていて、詳細な説明とその家の外観や内観の写真が見れるようにしました。

「使いやすさ」としてはページの表示速度や問い合わせフォームの使い勝手、ユーザーが欲しい情報に早く辿り着けるかの導線などユーザーに余計な手間をかけさせない工夫を意識しました。ページの表示速度はReactを用いることによってWebページを更新する際、仮想DOM を用いることで全体ではなく、変更箇所だけを更新するため高速に動作するようにしました。また画像・動画サイズの圧縮でページの高速化をしました。問い合わせフォームは email js を用いていてバリデーションチェックと、送信が成功した時には「送信に成功しました」、失敗した時には「送信に失敗しました」とアラートを出すようにしています。またわざわざ他のメールアプリケーションから送る手間をなくすために直接ページ内からメールを送れるようにしました。導線はこのWebサイトを見て来てくれる方は「どんな住宅を手がけてきたのか」が一番気になるのではないかと思い、トップページの下に住宅事例を載せているWorksセクションを配置しました。またリメイク前のサイトで「理念(Philosophy)」「設計の流れ(Flow)」「事務所概要(Overview)」セクションをクリックしてとぶ方が少なかったことと、離脱率が他セクションよりも高かったことから、思い切って案内するナビゲーションを下の方に配置し、より見てもらいたい住宅事例を載せたWorksセクションと建築士を紹介しているAboutセクションを上部に配置しました。その効果もあってか、全体の離脱率が下がり前のサイトよりも集客数が向上しました。またレスポンシブにも対応していてどのデバイスでも快適に見ることができるようになっています。
...
「見た目の印象」は全体的に今風のカッコ良いモダンな印象を与えたいということで全体的に背景の写真、動画に黒いオーバーレイをかけてスタイリッシュなリッチに見えるような効果を出しました。しかし動きは少なめで見やすいシンプルなサイトにしたいということだったのでそのデザインで制作をしました。全体的なデザインのアイデアはdhu webとONEOKROCK webを元に出していて、他にも事務所概要のアコーディオンメニューや設計の流れの数字を左側に統一して見やすいようになど様々な要素を加えました。動きはホバーすると動きが出るぐらいで凝ったアニメーションは使わないようにしました。背景は単色の色を使うのではなく、グラデーションだったり、オーバーレイがかかっている動画や画像を用いることによって「かっこいいな」「凝っているな」「新しいな」と思ってもらえるような工夫をしました。トップページは離脱率が減りそうなインパクトのあるものが良いということで、その時に完成間近だった南房総の家をドローンで撮影してもらいそれをトップページに動画で載せました。その動画も動画サイズを高画質のまま極限まで圧縮し、できるだけストレスなく高速に見れるようにしています。
...
改善点としてはPageSpeed Insightsを用いて高得点を出すように改善を繰り返していますが、トップページに動画を用いているのもあり、携帯サイズでの初期表示が遅くなってしまっていて、ユーザー補助、おすすめの方法、SEOは高得点を出しているのですが、パフォーマンスが悪いので今後改善していきたいと思っています。また時間があるときにNextjsへの移行を考えています。最初の描画速度が遅いのをNextjs特有のハイブリッドなレンダリング手法を用いることによってある程度改善できると思ったためです。
